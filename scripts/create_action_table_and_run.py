...
- emergency_routing (action table with completion + output columns)
...
def create_table_payloads():
    # Action table: emergency_routing
    emergency = {
        ...
        "cols": [
            {"id": "input", "name": "Input", "type": "text"},
            {
                ...
                "config": {
                    "completion": {
                        "system_instruction": (
                            "You are a structured data decoder. "
                            "Use the 'input' field and return ONLY a comma-separated list of tags "
                            "like '4 Pax, Warga Emas/Bedridden, Pet/Cat'."
                        )
                    }
                }
            },
...
def create_table(payload):
    ...
    print(f'Creating table via POST {url} payload.id={payload.get("id")}')
    try:
